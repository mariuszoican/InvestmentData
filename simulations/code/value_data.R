...
p_ce <- ggplot(df_ce, aes(x = CE)) +
  ...
  theme(
    ...
    panel.grid.minor = element_blank(),
    legend.position.inside = c(0.80, 0.82),          # inside the plot
...
if (!dir.exists("generated_data/plots")) {
  dir.create("generated_data/plots", recursive = TRUE)
}

print(combined_plot)

ggsave(filename = "generated_data/plots/info_value_panels.png",
  plot = combined_plot,
  width = 16,
  height = 9,
  dpi = 300
